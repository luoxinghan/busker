...
class BuskerUpdate extends Component{
    ...
    render() {
        ...
        const {imageUrl, currentBusker, data, redirectTo} = this.props;
        if (data.get("code") === 0) {
            return (
                            ...
                            <Form {...formItemLayout} onSubmit={this.handleSubmit}>
                                <Form.Item label="Nickname">
                                    {getFieldDecorator('nickname', {
                                        initialValue: currentBusker === null ? null : currentBusker.get("buskerName"),
                                        rules: [{required: true, message: 'Please input your nickname!'}],
                                    })(<Input/>)}
                                </Form.Item>
                                <Form.Item label="Sex">
                                    {getFieldDecorator('sex', {
                                        initialValue: currentBusker === null ? null : currentBusker.get("sex"),
                                ...
                                <Form.Item label="Birthday">
                                    {getFieldDecorator('birth', {
                                        initialValue: currentBusker === null ? null : moment(currentBusker.get("age"), "YYYY-MM-DD"),
                                        rules: [{
                                            type: 'object',
                                            required: true,
                              ...
                              Instruments&nbsp;
                                ...
                                >
                                    {getFieldDecorator('instrument', {
                                        initialValue: currentBusker === null ? null : currentBusker.get("instrument")
                                    })(<Input/>)}
                                </Form.Item>
                                <Form.Item label="Introduce">
                                    {getFieldDecorator('introduce', {
                                        initialValue: currentBusker === null ? null : currentBusker.get("introduce")
                                    })(<Input.TextArea/>)}
                                </Form.Item>
                                <Form.Item
                                    wrapperCol={{
...
const mapDispatchToProps = (dispatch) => {
    return {
        getBusker(){
            dispatch(actionCreators.getBusker());
        },
